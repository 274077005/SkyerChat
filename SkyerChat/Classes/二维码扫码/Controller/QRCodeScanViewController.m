...
- (void)viewDidLoad {
    ...
    @weakify(self)
    [[[self skCreatBtn:@"bar-返回-白" btnTitleOrImage:(btntypeImage) btnLeftOrRight:(btnStateLeft)] rac_signalForControlEvents:(UIControlEventTouchUpInside)] subscribeNext:^(__kindof UIControl * _Nullable x) {
        @strongify(self)
...
- (void)makeConstraintsForUI {
    ...
    [_view_bg mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(0.7 * skScreenWidth,  0.5 * (skScreenHeight - 64)));
        make.left.mas_equalTo(@(0.15 * skScreenWidth));
        
        make.top.mas_equalTo(@(0.25 * (skScreenHeight - 64) - 32));
    }];
    
    [_lab_word mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.size.mas_equalTo(CGSizeMake(skScreenWidth, 21));
...
- (void)startScan {
    ...
    _previewLayer.frame = CGRectMake(0, 0, skScreenWidth, skScreenHeight - 64);
...
- (void)scanLineMove {
    ...
    animation.toValue = [NSValue valueWithCGPoint:CGPointMake(0, 0.5 * (skScreenHeight - 64))];
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    ...
    [self skScanResult:stringValue];
    [self.navigationController popViewControllerAnimated:YES];
    
}
-(void)skScanResult:(NSString *)result{
...
- (CALayer *)layer_scanLine {
    ...
    if (!_layer_scanLine) {
        ...
        layer.bounds = CGRectMake(0, 0, 0.7 * skScreenWidth, 1);
...
- (UILabel *)lab_word {
    ...
    if (!_lab_word) {
        ...
        _lab_word.font = [UIFont systemFontOfSize:12];
